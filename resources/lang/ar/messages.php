<?php

return [
    'welcome' => 'مرحباً',
    'login' => 'تسجيل الدخول',
    'logout' => 'تسجيل الخروج',
    'register' => 'انشئ حساب',
    'create' => 'انشاء',
    'email_address' => 'الايميل',
    'password' => 'كلمة المرور',
    'remember_me' => 'تذكرني',
    'forgot_your_password' => 'هل نسيت كلمة المرور؟',
    'dont_have_an_account' => 'ليس لديك حساب؟',
    'the_provided_credentials_do_not_match_our_records' => 'البيانات غير صحيحة.',
    'registration_successful_please_login' => 'تم انشاء الحساب بنجاح. الرجاء تسجيل الدخول.',
    'client_login' => 'دخول زبون',
    'client_login_subtitle' => 'طلباتك وخدمات المغسلة',
    'employee_login' => 'دخول موظف',
    'employee_login_subtitle' => 'إدارة الطلبات والفواتير',
    'driver_login' => 'دخول سائق',
    'driver_login_subtitle' => 'إدارة التوصيل والاستلام',
    'admin_login' => 'دخول ادارة',
    'admin_login_subtitle' => 'صلاحيات كاملة وإدارة النظام',
    'client_register' => 'انشاء حساب زبون',
    'admin_register' => 'انشاء حساب ادارة',
    'name' => 'الاسم',
    'email' => 'الايميل',
    'mobile_no' => 'رقم الجوال',
    'address' => 'العنوان',
    'confirm_password' => 'تاكيد كلمة المرور',
    'select_address' => 'اختر العنوان',
    'select_province' => 'اختر المحافظة',
    'select_city' => 'اختر المدينة',
    'province' => 'المحافظة',
    'city' => 'المدينة',
    'address' => 'العنوان',
    'manage_users' => 'ادارة المستخدمين',
    'admin_dashboard' => 'واجهة الادارة',
    'no_data_to_display' => 'لا توجد بيانات لعرضها',
        'users_by_balance' => 'المستخدمون حسب الرصيد',
        'all_balances' => 'كل الارصدة',
        'positive_balance' => 'رصيد موجب',
        'zero_balance' => 'رصيد صفري',
        'all_points' => 'كل النقاط',
        'positive_points' => 'لديه نقاط',
        'zero_points' => 'بدون نقاط',
    'mobile' => 'رقم الجوال',
    'created_at' => 'تاريخ الانشاء',
    'updated_at' => 'تاريخ التعديل',
    'user_type' => 'نوع المستخدم',
    'client' => 'زبون',
    'admin' => 'ادارة',
    'employee' => 'موظف',
    'driver' => 'السائق',
    'modify' => 'تعديل',
    'updated_successfully' => 'تم تعديل البيانات بنجاح',
    'add' => 'اضافة',
    'delete' => 'حذف',
    'deleted_successfully' => 'تم الحذف بنجاح',
    'confirm_deletion' => 'هل انت متاكد انك تريد الحذف؟',
    'cannot_delete_last_admin' => 'لا يمكنك حذف آخر مسؤول.',
    'delete_failed' => 'فشلت عملية الحذف.',
    'registration_failed' => 'فشل انشاء المستخدم. يرجى المحاولة مرة أخرى.',
    '404_page_not_found' => 'الصفحة الذي تحاول الوصول اليها غير موجودة',
    '419_page_expired' => 'انتهت صلاحية الصفحة',
    'go_homepage' => 'الذهاب للرئيسية',
    'manage_orders' => 'ادارة الطلبات',
    'create_order' => 'انشاء طلب',
    'user' => 'المستخدم',
    'select_user' => 'اختر المستخدم',
    'select' => 'اختر',
    'discount' => 'الخصم',
    'discount_amount' => 'قيمة الخصم',
    'subscription' => 'الاشتراك',
    'subscriptions' => 'الاشتراكات',
    'total_price' => 'المبلغ الكلي',
    'status' => 'الحالة',
    'order_products' => 'منتجات الطلب',
    'product' => 'المنتج',
    'products' => 'المنتجات',
    'product_service' => 'خدمة المنتج',
    'quantity' => 'الكمية',
    'select_subscription' => 'اختر الاشتراك',
    'pending' => 'قيد الانتظار',
    'create_product' => 'انشاء منتج',
    'product_details' => 'تفاصيل المنتج',
    'back' => 'الرجوع',
    'show' => 'عرض',
    'view' => 'عرض',
    'edit' => 'تعديل',
    'update' => 'تحديث',
    'created_successfully' => 'تم الانشاء بنجاح',
    'updated_successfully' => 'تم التحديث بنجاح',
    'deleted_successfully' => 'تم الحذف بنجاح',
    'orders' => 'الطلبات',
    'users' => 'المستخدمين',
    'manage_products' => 'ادارة المنتجات',
    'product_services' => 'خدمات المنتج',
    'manage_product_services' => 'ادارة خدمات المنتج',
    'price' => 'السعر',
    'cancel' => 'الغاء',
    'search' => 'بحث',
    'search_product' => 'البحث عن منتج (بالاسم او الاي دي)',
    'search_user' => 'البحث عن مستخدم (بالاسم او الاي دي او رقم الجوال او الايميل)',
    'hello' => 'مرحبا',
    'select_product' => 'اختر المنتج',
    'select_product_service' => 'اختر خدمة المنتج',
    'select_discount' => 'اختر الخصم',
    'save' => 'حفظ',
    'total_price' => 'السعر الكلي',
    'manage_subscriptions' => 'ادارة الاشتراكات',
    'paid' => 'مدفوع',
    'benefit' => 'ممنوح',
    'start_date' => 'تاريخ البدء',
    'end_date' => 'تاريخ الانتهاء',
    'period' => 'المدة',
    'period_duration' => 'مدة الاشتراك',
    'period_unit' => 'وحدة المدة',
    'period_day' => 'يوم',
    'period_day_plural' => 'أيام',
    'period_week' => 'أسبوع',
    'period_week_plural' => 'أسابيع',
    'period_month' => 'شهر',
    'period_month_plural' => 'أشهر',
    'period_year' => 'سنة',
    'period_year_plural' => 'سنوات',
    'subscription_expired' => 'منتهي',
    'subscription_active' => 'نشط',
    'subscription_client_has_active' => 'هذا الزبون لديه اشتراك نشط بالفعل. لا يمكن أن يكون له أكثر من اشتراك نشط في نفس الوقت.',
    'subscription_client_used_plan' => 'لا يمكن لهذا الزبون الاشتراك في هذه الخطة مرة أخرى. لقد استخدم هذا الاشتراك سابقاً وانتهت مدته.',
    'subscription_no_available_plans' => 'لا توجد خطط اشتراك متاحة. لديك اشتراك نشط بالفعل، أو أنك استخدمت جميع الخطط.',
    'period_help_text' => 'الفترة تحدد عدد مرات تجديد هذا الاشتراك وخصمه — مثلاً "شهر واحد" تعني أنه يتم تحصيل المبلغ من المستخدم وإعادة إضافة المكافأة تلقائيًا كل شهر.',
    'subscription_billing_status' => 'حالة الدفع',
    'subscription_status_active' => 'نشط',
    'subscription_status_failed_once' => 'فشل الدفع (فترة واحدة)',
    'subscription_status_failed_multiple' => 'فشل الدفع (فترتين أو أكثر)',
    'subscription_status_none' => 'لا يوجد اشتراك',
    'subscription_status_suspended' => 'معلّق',
    'subscription_status_pending_payment' => 'بانتظار الدفع',
    'next_billing_date' => 'تاريخ الدفع القادم',
    'subscriptions_report' => 'تقرير الاشتراكات',
    'subscriptions_report_desc' => 'معرفة عدد المستخدمين ذوي الاشتراكات النشطة أو الفاشلة',
    'subscriptions_report_subtitle' => 'تتبع من قام بالدفع، ومن فاتته دفعة، ومن يحتاج إلى متابعة.',
    'of_total' => 'من الإجمالي',
    'total_subscriptions' => 'إجمالي الاشتراكات',
    'active_subscriptions' => 'نشطة',
    'failed_once_subscriptions' => 'فشلت (شهر واحد)',
    'failed_multiple_subscriptions' => 'فشلت (شهرين أو أكثر)',
    'filter_all' => 'الكل',
    'consecutive_failures' => 'عدد مرات الفشل المتتالية',
    'last_billed_at' => 'آخر عملية دفع',
    'create_subscription' => 'انشاء الاشتراك',
    'edit_subscription' => 'تعديل الاشتراك',
    'subscription_details' => 'تفاصيل الاشتراك',
    'manage_client_subscriptions' => 'ادارة اشتراكات المستخدمين',
    'add_client_subscription' => 'اضافة اشتراك لمستخدم',
    'date' => 'التاريخ',
    'edit_client_subscription' => 'تعديل اشتراك الزبون',
    'delivery' => 'التوصيل',
    'bring_order' => 'احضار الطلب',
    'return_order' => 'ارجاع الطلب',
    'delivery_price' => 'سعر التوصيل',
    'order_placed_balance' => 'تم تأكيد طلبك بنجاح! رصيدك الجديد هو: :balance',
    'processing' => 'جاري العمل',
    'shipped' => 'جاري التوصيل',
    'completed' => 'تمت الطلبية',
    'cancelled' => 'تم الالغاء',
    'select_driver' => 'اختر السائق',
    'edit_order' => 'تعديل الطلب',
    'order_details' => 'تفاصيل الطلب',
    'order_created_successfully' => 'تم انشاء الطلب بنجاح.',
    'order_updated_successfully' => 'تم تحديث الطلب بنجاح',
    'order_update_balance' => 'تم تحديث طلبك. رصيدك الجديد هو: :balance',
    'order_deleted_balance' => 'تم إلغاء طلبك. تم استرداد الرصيد. الرصيد الجديد: :balance',
    'search_order' => 'بحث عن طلب',
    'street' => 'الشارع',
    'building' => 'المبنى',
    'floor' => 'الطابق',
    'appartment_number' => 'رقم الشقة',
    'order_assignment' => 'توجيه الطلب',
    'select_order' => 'اختار الطلب',
    'recommend_driver' => 'اقترح سائق',
    'order_assigned_successfully' => 'تم توجيه الطلب بنجاح.',
    'order_status' => 'حالة الطلب',
    'order_status_placed' => 'تم الطلب',
    'order_status_pickup_scheduled' => 'تم جدولة الاستلام',
    'order_status_at_facility' => 'وصل إلى المغسلة',
    'order_status_sorting' => 'قيد الفرز',
    'order_status_washing' => 'قيد الغسيل',
    'order_status_ready_for_delivery' => 'جاهز للتوصيل',
    'order_status_out_for_delivery' => 'قيد التوصيل',
    'order_status_delivered' => 'تم التوصيل',
    'order_status_cancelled' => 'ملغي',
    'status_change_note' => 'ملاحظة تغيير الحالة',
    'order_notes' => 'ملاحظات',
    'order_notes_placeholder' => 'ملاحظات اختيارية حول هذا الطلب...',
    'balance' => 'الرصيد',
    'contact_messages' => 'رسائل التواصل',
    'no_contact_messages' => 'لا توجد رسائل تواصل',
    'id' => 'الرقم',
    'title' => 'العنوان',
    'message' => 'الرسالة',
    'anonymous' => 'مجهول',
    'read' => 'مقروءة',
    'replied' => 'تم الرد',
    'actions' => 'الإجراءات',
    'yes' => 'نعم',
    'no' => 'لا',
    'mark_read' => 'وضع علامة مقروءة',
    'mark_unread' => 'وضع علامة غير مقروءة',
    'mark_replied' => 'وضع علامة تم الرد',
    'mark_unreplied' => 'وضع علامة لم يتم الرد',
    'contact_status_updated' => 'تم تحديث حالة الرسالة',
    'contact_form' => 'نموذج التواصل',
    'send' => 'إرسال',
    'contact_sent' => 'تم إرسال رسالة التواصل بنجاح',
    'contact_messages' => 'رسائل التواصل',
    'no_contact_messages' => 'لا توجد اي رسائل تواصل',
    'send_message' => 'ارسال رسالة',
    'contact_message_details' => 'تفاصيل الرسالة',
    'reply' => 'الرد',
    'enter_reply' => 'أدخل الرد',
    'send_reply' => 'إرسال الرد',
    'reply_sent' => 'تم إرسال الرد بنجاح',
    'previous_replies' => 'الردود السابقة',
    'search' => 'بحث',
    'all_read_status' => 'جميع حالات القراءة',
    'unread' => 'غير مقروءة',
    'all_reply_status' => 'جميع حالات الرد',
    'unreplied' => 'لم يتم الرد',
    'filter' => 'فلترة',
    'reply_disabled_anonymous' => 'الرد غير متاح للرسائل المجهولة.',
    'users_by_Number' => 'المستخدمون حسب رقم الجوال',
    'all_user_types' => 'جميع أنواع المستخدمين',
    'whatsapp_message' => 'رسالة واتساب',
    'send_whatsapp' => 'إرسال واتساب',
    'whatsapp_messages_sent' => 'تم إرسال رسائل واتساب',
    'no_users_found' => 'لم يتم العثور على مستخدمين',
    'send_whatsapp_filtered' => 'ارسال واتساب لجميع النتائج',
    'negative_balance' => 'رصيد سالب',
    'send_negative_balance_message' => 'إرسال رسالة لاصحاب الرصيد السالب',
    'negative_balance_reminder' => 'يرجى إعادة شحن رصيدك.',
    'delivery_options' =>'خيارت التوصيل',
    'unauthorized_access' => 'لا يمكن الوصول الى هذا المحتوى',
    'manage_my_client_subscriptions' => 'ادارة اشتراكاتي',
    'add_subscription' => 'اضافة اشتراك',
    'bills' => 'الفواتير',
    'client_orders_and_balance' => 'طلبات المستخدم والرصيد',
    'orders_with_billing_info' => 'الطلبات وبيانات الدفع',
    'order_id' => 'رقم الطلب',
    'balance_summary'=>'ملخص الرصيد',
    'total_billed' => 'مجمل الفواتير',
    'total_paid' => 'مجموع الدفع',
    'current_balance' => 'الرصيد الحالي',
    'you_have_a_credit_of' => 'رصيدك الحالي',
    'make_payment' => 'قم بالدفع',
    'pay_via_knet' => 'الدفع الآمن عبر كي نت (الكويت)',
    'subscription_created_pending_payment' => 'تم إنشاء الاشتراك بنجاح. سيتم تفعيله بمجرد قيام المستخدم بإتمام الدفع عبر كي نت.',
    'you_owe' => 'انت مدين ب',
    'your_balance_is' => 'رصيدك هو',
    'balance_negative_message' => 'رصيدك الحالي بالسالب. الرجاء القيام بالدفع.',
    'add_balance' => 'اضافة رصيد',
    'current_orders' => 'الطلبات الحالية',
    'delivery_orders'=>'طلبات التوصيل',
    'no_orders_found' => 'لا توجد اي طلبات',
    'change_status' => 'تغيير الحالة',
    'change_status_to_completed' => 'تغيير الحالة الى تم التوصيل',
    'change_status_to_shipped' => 'تغيير الحالة الى جاري التوصيل',
    'required_delivery' => 'التوصيل المطلوب',
    'delivery_history' => 'تاريخ الطلبات',
    'reset_filter' => 'مسح الفلتر',
    'balance_positive_message' => 'لا تحتاج الى اضافة رصيد',
    
    // Discount Feature Translations
    'apply_discount' => 'تطبيق الخصم',
    'discount_type' => 'نوع الخصم',
    'fixed_amount' => 'مبلغ ثابت',
    'percentage' => 'نسبة مئوية',
    'discount_value' => 'قيمة الخصم',
    'discount_preview' => 'معاينة الخصم',
    'discount_amount_calculated' => 'قيمة الخصم المحسوبة',
    'new_subtotal' => 'المجموع الفرعي الجديد',
    'new_total' => 'الإجمالي الجديد',
    'savings' => 'المدخرات',
    'clear_discount' => 'مسح الخصم',
    'remove_discount' => 'إزالة الخصم',
    'discount_applied_by' => 'تم تطبيق الخصم بواسطة',
    'discount_applied' => 'تم تطبيق الخصم',
    'discount_removed' => 'تم إزالة الخصم',
    'order_subtotal' => 'المجموع الفرعي للطلب',
    'discount_validation_exceeds_subtotal' => 'لا يمكن أن يتجاوز الخصم الثابت المجموع الفرعي للطلب',
    'discount_validation_exceeds_100_percent' => 'لا يمكن أن يتجاوز الخصم النسبي 100%',
    'discount_validation_positive' => 'يجب أن تكون قيمة الخصم موجبة',
    'discount_no_discount_found' => 'لا يوجد خصم على هذا الطلب',
    'discount_invalid_status' => 'لا يمكن تطبيق الخصم على هذه الحالة من الطلب',
    'discount_history_applied_by' => 'تم التطبيق بواسطة',
    'discount_history_applied_at' => 'تم التطبيق في',
    'discount_you_saved' => 'لقد وفرت',
    'discount_display_fixed' => 'مبلغ ثابت',
    'discount_display_percentage' => 'نسبة مئوية',
    'discount_validation_failed' => 'فشل التحقق من الخصم',
    'discount_valid' => 'الخصم صالح',
    
    // Currency
    'currency_symbol' => 'د.ك',
    'off' => 'خصم',
    'tax' => 'الضريبة',
    
    // Delivery Information
    'delivery_information' => 'معلومات التوصيل',
    'delivery_direction' => 'اتجاه التوصيل',
    'not_assigned' => 'غير معين',
    
    // Driver/Delivery Validation
    'driver_required' => 'يرجى اختيار سائق عند تحديد خيار التوصيل.',
    'delivery_option_required' => 'يرجى تحديد خيار توصيل واحد على الأقل (إحضار الطلب أو إرجاع الطلب) عند اختيار سائق.',
    'clear' => 'مسح',
    'delivery_price_zero_warning' => 'يرجى تحديث سعر التوصيل - تم تعيينه حالياً إلى 0.',
    
    // User Details
    'user_details' => 'تفاصيل المستخدم',
    'user_information' => 'معلومات المستخدم',
    'balance_information' => 'معلومات الرصيد',
    'total_orders' => 'إجمالي الطلبات',
    'pending_orders' => 'الطلبات المعلقة',
    'completed_orders' => 'الطلبات المكتملة',
    'total_spent' => 'إجمالي الإنفاق',
    'recent_orders' => 'الطلبات الأخيرة',

    // Product Image
    'image' => 'الصورة',
    'delete_image' => 'حذف الصورة',
    'image_deleted' => 'تم حذف الصورة بنجاح',

    // Service Pricing
    'service_pricing' => 'تسعير الخدمات',
    'service_pricing_help' => 'تكوين الأسعار لكل خدمة سيقدمها هذا المنتج',
    'service_pricing_info' => 'اختر الخدمات التي سيقدمها هذا المنتج وقم بتعيين أسعارها. يمكنك ترك الخدمات غير محددة إذا لم تنطبق على هذا المنتج.',
    'at_least_one_service' => 'يرجى تحديد خدمة واحدة على الأقل لهذا المنتج',
    'price_required' => 'السعر مطلوب ويجب أن يكون أكبر من 0',
    'services' => 'خدمات',
    'services_configured' => 'خدمات مهيأة',
    'no_services' => 'لا توجد خدمات',
    'no_services_configured' => 'لا توجد خدمات مهيأة',
    'configure_services' => 'تكوين الخدمات',
    'product_no_services_warning' => 'هذا المنتج ليس لديه خدمات مهيأة. يرجى تكوين الخدمات قبل الإضافة إلى الطلبات.',
    'price' => 'السعر',
    'select_product' => 'اختر المنتج',
    'choose_product' => 'اختر منتج...',
    'select_service' => 'اختر الخدمة',
    'choose_service' => 'اختر خدمة...',
    'service_price' => 'سعر الخدمة',
    'enable_service' => 'تفعيل الخدمة',

    // Order form errors
    'please_fix_errors' => 'يرجى تصحيح الأخطاء التالية قبل الإرسال:',
    'order_error_try_again' => 'حدث خطأ أثناء إنشاء الطلب. يرجى التحقق من المدخلات والمحاولة مرة أخرى.',

    // Order Validation Messages
    'validation_user_required' => 'يرجى اختيار مستخدم لهذا الطلب.',
    'validation_user_exists' => 'المستخدم المحدد غير موجود.',
    'validation_driver_required' => 'يرجى اختيار سائق عند تحديد خيار التوصيل.',
    'validation_driver_exists' => 'السائق المحدد غير موجود.',
    'validation_province_required' => 'يرجى اختيار المحافظة لعنوان التوصيل.',
    'validation_city_required' => 'يرجى اختيار المدينة لعنوان التوصيل.',
    'validation_street_required' => 'يرجى إدخال الشارع لعنوان التوصيل.',
    'validation_building_required' => 'يرجى إدخال المبنى لعنوان التوصيل.',
    'validation_floor_required' => 'يرجى إدخال رقم الطابق لعنوان التوصيل.',
    'validation_floor_integer' => 'يجب أن يكون رقم الطابق عدداً صحيحاً.',
    'validation_apartment_required' => 'يرجى إدخال رقم الشقة لعنوان التوصيل.',
    'validation_products_required' => 'يرجى إضافة منتج واحد على الأقل للطلب.',
    'validation_product_required' => 'يرجى اختيار منتج في الصف :row.',
    'validation_product_exists' => 'المنتج المحدد في الصف :row غير موجود.',
    'validation_service_required' => 'يرجى اختيار خدمة في الصف :row.',
    'validation_service_exists' => 'الخدمة المحددة في الصف :row غير موجودة.',
    'validation_quantity_required' => 'يرجى إدخال الكمية في الصف :row.',
    'validation_quantity_min' => 'يجب أن تكون الكمية في الصف :row واحداً على الأقل.',
    'validation_order_status_required' => 'يرجى اختيار حالة الطلب.',
    'validation_order_status_invalid' => 'حالة الطلب المحددة غير صالحة.',
    'validation_delivery_price_required' => 'يرجى إدخال سعر التوصيل.',
    'validation_delivery_price_numeric' => 'يجب أن يكون سعر التوصيل رقماً.',

    // Order Warnings
    'duplicate_product_service' => 'هذا المنتج والخدمة موجودان بالفعل في الطلب.',
    'user_required' => 'يرجى اختيار مستخدم لهذا الطلب.',

    // Dashboard Cards
    'employee_dashboard' => 'لوحة تحكم الموظف',
    'invoice_data' => 'بيانات الفواتير',
    'total_income' => 'إجمالي الدخل',
    'create_new_order' => 'إنشاء طلب جديد',
    'view_invoice_statistics' => 'عرض إحصائيات الفواتير',
    'view_income_reports' => 'عرض تقارير الدخل',
    'go_to_order' => 'الذهاب إلى طلب',
    'go_to_order_desc' => 'انتقل مباشرة إلى طلب باستخدام رقمه',
    'order_id_placeholder' => 'أدخل رقم الطلب',
    'order_not_found' => 'الطلب رقم :id غير موجود.',
    'go' => 'ذهاب',
    'go_to_user' => 'الذهاب إلى مستخدم',
    'go_to_user_desc' => 'انتقل مباشرة إلى مستخدم باستخدام رقمه',
    'user_id_placeholder' => 'أدخل رقم المستخدم',
    'user_not_found' => 'المستخدم رقم :id غير موجود.',

    // Total Income Page
    'income_overview' => 'نظرة عامة على الدخل',
    'this_month' => 'هذا الشهر',
    'average_order_value' => 'متوسط قيمة الطلب',
    'monthly_income' => 'الدخل الشهري',
    'monthly_income_chart' => 'مخطط الدخل الشهري (آخر 12 شهر)',
    'income_by_status' => 'تفصيل الدخل حسب الحالة',
    'order_count' => 'عدد الطلبات',
    'total_amount' => 'المبلغ الإجمالي',
    'percentage_share' => 'النسبة',
    'top_orders' => 'أعلى الطلبات قيمة',
    'back_to_dashboard' => 'العودة للوحة التحكم',
    'no_income_data' => 'لا توجد بيانات دخل متاحة بعد.',
    'income' => 'الدخل',

    // Invoice Data Page
    'invoice_overview' => 'نظرة عامة على الفواتير',
    'total_invoices' => 'إجمالي الفواتير',
    'total_revenue' => 'إجمالي الإيرادات',
    'avg_invoice_value' => 'متوسط قيمة الفاتورة',
    'search_invoices' => 'البحث برقم الطلب أو اسم العميل',
    'items_count' => 'العناصر',
    'subtotal' => 'المجموع الفرعي',
    'delivery_cost' => 'التوصيل',
    'total' => 'الإجمالي',
    'invoice_items' => 'عناصر الفاتورة',
    'unit_price' => 'سعر الوحدة',
    'line_total' => 'إجمالي السطر',
    'service' => 'الخدمة',
    'no_invoices_found' => 'لم يتم العثور على فواتير تطابق معاييرك.',
    'show_details' => 'عرض التفاصيل',
    'hide_details' => 'إخفاء التفاصيل',
    'print_invoice' => 'طباعة',
    'from_date' => 'من',
    'to_date' => 'إلى',
    'all_statuses' => 'جميع الحالات',
    'filter_results' => 'تصفية',
    'clear_filters' => 'مسح',

    // Notification templates (WhatsApp)
    'subscription_balance_added' => 'تمت إضافة مكافأة الاشتراك! رصيدك الجديد: :balance',
    'subscription_renewal_success' => 'تم تجديد اشتراكك! تم خصم :amount د.ك، رصيدك الجديد: :balance',
    'subscription_renewal_failed' => 'فشل تجديد اشتراكك بسبب عدم كفاية الرصيد. يرجى شحن رصيدك. رصيدك الحالي: :balance',
    'subscription_initial_payment_link' => 'تم إنشاء اشتراك لحسابك. يرجى دفع :amount د.ك عبر هذا الرابط لتفعيله: :payment_url',
    'subscription_renewal_payment_link' => 'تعذّر خصم :amount د.ك لتجديد اشتراكك من رصيد المحفظة. يرجى الدفع عبر الرابط للحفاظ على اشتراكك: :payment_url',
    'subscription_suspended' => 'تم تعليق اشتراكك بسبب عدم سداد :amount د.ك لفترات متعددة. يرجى التواصل معنا لإعادة التفعيل.',
    'subscription_activated' => 'مفعّل',
    'driver_delivery_completed' => 'تم التوصيل. تمت إضافة رسوم التوصيل :amount د.ك لرصيدك. الرصيد الجديد: :balance',

    // Client settings
    'settings' => 'الإعدادات',
    'notification_language' => 'لغة الإشعارات',
    'notification_language_help' => 'اختر اللغة لإشعارات الواتساب والمعاملات',
    'settings_updated' => 'تم تحديث الإعدادات بنجاح',

    // Admin notifications
    'notification_templates' => 'رسائل الإشعارات',
    'notification_templates_help' => 'تخصيص رسائل إشعارات الواتساب المرسلة للمستخدمين',
    'message_key' => 'مفتاح الرسالة',
    'message_arabic' => 'الرسالة (عربي)',
    'message_english' => 'الرسالة (إنجليزي)',

    // Payment
    'payment_amount' => 'مبلغ الدفع',
    'pay_now' => 'ادفع الآن',
    'payment_successful' => 'تمت عملية الدفع بنجاح',
    'payment_failed' => 'فشلت عملية الدفع',
    'payment_thank_you' => 'شكراً لك! تم معالجة الدفع بنجاح.',
    'payment_failed_message' => 'لم يتم معالجة الدفع. يرجى المحاولة مرة أخرى.',
    'payment_method_label' => 'طريقة الدفع',
    'payment_reference' => 'المرجع',
    'try_again' => 'حاول مرة أخرى',
    'payment_completed' => 'تم استلام دفعة بمبلغ :amount د.ك. رصيدك الجديد: :balance',
    'payment_history' => 'سجل المدفوعات',

    // Points System
    'points' => 'النقاط',
    'points_balance' => 'رصيد النقاط',
    'points_price' => 'سعر النقاط',
    'points_per_unit' => 'النقاط لكل وحدة',
    'points_price_help' => 'اتركه فارغاً إذا كانت هذه الخدمة لا يمكن دفعها بالنقاط.',
    'points_optional' => 'النقاط (اختياري)',
    'total_points' => 'إجمالي النقاط',
    'points_not_available' => 'غير قابل للدفع بالنقاط',
    'points_redeemed' => 'النقاط المستبدلة',
    'income_excludes_points' => 'لا يشمل الطلبات المدفوعة بالنقاط',
    'points_packages' => 'باقات النقاط',
    'manage_points_packages' => 'إدارة باقات النقاط',
    'create_points_package' => 'إضافة باقة نقاط',
    'edit_points_package' => 'تعديل باقة النقاط',
    'buy_points' => 'شراء نقاط',
    'assign_points' => 'منح نقاط',
    'points_purchase_history' => 'سجل شراء النقاط',
    'points_added' => 'تمت إضافة النقاط! رصيد نقاطك الجديد: :points_balance نقطة',
    'my_points' => 'نقاطي',
    'pay_with_money' => 'الدفع بالمال (د.ك)',
    'pay_with_points' => 'الدفع بالنقاط',
    'pay_with_knet' => 'الدفع بـ KNET',
    'knet_sandbox' => 'اختبار',
    'knet_payment_initiation_failed' => 'فشل بدء الدفع عبر KNET. يرجى المحاولة مرة أخرى.',
    'retry_order_payment' => 'إعادة محاولة الدفع',
    'not_paid' => 'غير مدفوع',
    'payment_status' => 'حالة الدفع',
    'pay_now' => 'ادفع الآن',
    'order_paid_successfully' => 'تمت عملية الدفع بنجاح.',
    'order_already_paid' => 'تم دفع هذا الطلب مسبقاً.',
    'select_payment_method' => 'يرجى اختيار طريقة الدفع.',
    'settle_balance' => 'تسوية الرصيد',
    'outstanding_balance' => 'الرصيد المستحق',
    'amount' => 'المبلغ',
    'balance_settled_successfully' => 'تمت تسوية الرصيد بنجاح.',
    'no_outstanding_balance' => 'لا يوجد رصيد مستحق على هذا المستخدم.',
    'create_order_for_user' => 'إنشاء طلب لهذا المستخدم',
    'copy_payment_link' => 'نسخ رابط الدفع',
    'payment_link_copied' => 'تم نسخ رابط الدفع!',
    'payment_link_description' => 'أرسل هذا الرابط إلى العميل للدفع عبر KNET.',
    'points_used' => 'النقاط المستخدمة',
    'package_price_kwd' => 'سعر الباقة (د.ك)',
    'package_points' => 'نقاط الباقة',
    'package_name' => 'اسم الباقة',
    'insufficient_points' => 'رصيد النقاط غير كافٍ لإتمام هذا الطلب.',
    'points_assigned_successfully' => 'تمت إضافة النقاط بنجاح إلى :user (:points نقطة).',
    'payment_initiation_failed' => 'فشل بدء الدفع. يرجى المحاولة مرة أخرى.',
    'select_package' => 'اختر الباقة',
    'current_points_balance' => 'رصيد النقاط الحالي',
    'points_system' => 'النقاط',
    'active' => 'نشط',
    'inactive' => 'غير نشط',
    'added_by' => 'أضيف بواسطة',
    'created_at' => 'تاريخ الإنشاء',
    'description' => 'الوصف',
    'optional' => 'اختياري',
    'product_no_points_price_warning' => 'خدمة واحدة أو أكثر من الخدمات المختارة لا تحتوي على سعر نقاط. يرجى تعيين سعر النقاط للخدمة قبل الدفع بالنقاط.',
    'no_data' => 'لا توجد بيانات.',
    'payment_success' => 'تم الدفع بنجاح',
    'payment_failed_description' => 'تعذّر معالجة دفعتك. يرجى المحاولة مرة أخرى.',
    'retry_payment' => 'حاول مجدداً',
    'payment_link_thank_you' => 'شكراً! تم استلام دفعتك.',
    'transaction_id' => 'رقم المعاملة',

    // إعادة الوزن / إعادة التسعير
    'reprice_only_at_facility' => 'لا يمكن إعادة تسعير الطلب إلا أثناء وجوده في المنشأة أو أثناء الفرز.',
    'order_repriced_successfully' => 'تم إعادة تسعير الطلب بنجاح.',
    'no_additional_payment_due' => 'لا يوجد مبلغ إضافي مستحق على هذا الطلب.',

    // New keys for contact messaging UI
    'new_message' => 'رسالة جديدة',
    'chat_with_support' => 'تحدث مع الدعم الفني',
    'type_message_here' => 'اكتب رسالتك هنا...',
    'no_replies_yet' => 'لا توجد ردود بعد',
    'admin_reply' => 'فريق الدعم الفني',
    'you' => 'أنت',
    'send_message_title' => 'تواصل مع الدعم',
];
