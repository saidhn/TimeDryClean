    ...
    <div class="row g-2">
        <div class="col-12">
            <div class="d-flex justify-content-between">
                ...
                <span>${{ number_format($order->sum_price, 2) }}</span>
            </div>
        </div>
        ...
        <div class="col-12">
            <div class="d-flex justify-content-between">
                ...
                <span>${{ number_format($order->sum_price - $order->discount_amount, 2) }}</span>
            </div>
        </div>
        @endif
        ...
        <div class="col-12">
            <div class="d-flex justify-content-between">
                ...
                <span class="{{ $order->hasDiscount() ? 'text-success' : '' }}">
                    ${{ number_format($order->total ?? ($order->sum_price + ($order->tax ?? 0)), 2) }}
                </span>
            </div>
        </div>
        ...
        <div class="col-12">
            <div class="alert alert-success py-2 px-3 mb-0 mt-2">
                <small>
                    ...
                    {{ __('messages.discount_you_saved') }} <strong>${{ number_format($order->discount_amount, 2) }}</strong> {{ __('messages.savings') }}!
                </small>
            </div>
        </div>
        @endif
