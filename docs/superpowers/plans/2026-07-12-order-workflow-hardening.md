...
class OrderStatusEnumTest extends OrderStatusEnumTestCase
{
}

abstract class OrderStatusEnumTestCase extends TestCase
{
...
```
(The indirection through an abstract base class is just to keep this a plain PHPUnit `TestCase`, not a Laravel `Tests\TestCase` — this test needs no DB/app boot, it's pure enum logic. Simplify to a single concrete class if your PHPUnit version complains about the split; the important content is the 5 test methods.)

Actually — simplify. Do not use the abstract-class indirection; PHPUnit runs concrete classes fine standalone. Replace the whole file with:
```php
<?php
